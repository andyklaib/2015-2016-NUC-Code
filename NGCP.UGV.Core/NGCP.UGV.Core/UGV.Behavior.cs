...
namespace NGCP.UGV.Core
{
    ...
    public partial class UGV
    {
        ...
        void GenerateSearchPath()
        {
            ...
            r *= distance;
            nextWayPoint = WayPoint.Projection(center, theta, r);
            WayPoint previousWaypoint = nextWayPoint;
            while (WayPoint.IsInsideBoundary(nextWayPoint.Lat, nextWayPoint.Long, Boundary))
            {
                distance += 0.000001;
                r = 1 + Math.Cos(k * theta);
            ...
            // ========= Predefined Points ======== //
            /*map = new List<WayPoint>()
            {
                new NGCP.Navigation.WayPoint(34.05930113611111, -117.82177103888888, 0)
            };*/
