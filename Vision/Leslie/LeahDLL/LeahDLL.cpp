...
void Leah::Mapping(uchar* data,int size,int angle, float bearing, float x,float y)
{
	Mat NewFrame = Mat::zeros(MaxRange * 2 * SCALE,MaxRange * 2 * SCALE,CV_8UC1);
	float slitAngle = size / angle;
	for(int i = 0; i < size;i++)
	{
		if(data[i] < MaxRange)
		{
			...
			if( X >= MaxRange * SCALE - 1)
			{
				X = (int)MaxRange * SCALE - 1;
			}
			else if(X <= 0)
			{
				X =0;
			}
			if( Y >= MaxRange * SCALE - 1)
			{
				Y = (int)MaxRange * SCALE - 1;
			}
			else if(Y <= 0)
			{
				Y = 0;
