...
namespace LisaControlerCsh
{
    class Rectangle
    {
        ...
        public bool CollisionRot(Line test, ref Vector2D hit)
        {
            ...
            float dis = 0;
            Vector2D intercept = new Vector2D();
            for (int i = 0; i < 4; i++)
            {
                if (test.Intesect(lines[i],ref intercept))
                {
                    if ((intercept.GetX() * lines[i].Slope + lines[i].b > lines[i].start.GetY() && intercept.GetX() * lines[i].Slope + lines[i].b < lines[i].end.GetY()) ||
                        (intercept.GetX() * lines[i].Slope + lines[i].b < lines[i].start.GetY() && intercept.GetX() * lines[i].Slope + lines[i].b > lines[i].end.GetY()))
                    {
                        dis = test.start.Distance(intercept);
                        if (dis < min)
                        {
                            min = dis;
                            hit = intercept;
