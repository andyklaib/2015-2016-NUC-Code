...
namespace NGCP.Navigation
{
    public class WayPoint
    {
        ...
        public static WayPoint Projection(WayPoint wp, double bearing, double distance)
        {
            ...
            tLat = distance * (Math.Cos(Math.PI - bearing)) + wp.Lat;
            tLong = distance * (Math.Sin(Math.PI - bearing)) + wp.Long;

            return new WayPoint(tLat, tLong, 0);*/

            // ==== old code for Projection ====
        ...
        public static bool IsInsideBoundary(double Lat, double Long, List<WayPoint> Boundary)
        {
            ...
            double checkArea = 0;
            double tempArea = 0;
            double[] x = new double[numCorners];
            double[] y = new double[numCorners];
            int counter = 0;
            int next = 0;

            foreach (var waypoint in Boundary)
            {
                x[counter] = waypoint.Lat;
                y[counter] = waypoint.Long;
                counter++;
            }

            recArea = Math.Abs((x[0] * y[1] - x[1] * y[0]) + (x[1] * y[2] - x[2] * y[1]) + (x[2] * y[3] - x[3] * y[2]) + (x[3] * y[0] - x[0] * y[3])) / 2;
            ...
            for (int i = 0; i < numCorners; i++)
            {
                ...
                if (next == numCorners)
                    next = 0;
                tempArea = Math.Abs(x[i] * (y[next] - Long) + x[next] * (Long - y[i]) + Lat * (y[i] - y[next])) / 2;
                if (tempArea == 0)
                    return false;
                checkArea += tempArea;
            }

            if (checkArea == recArea)
                return true;
            else
                return false;
            */

            // ==== old code for IsInsideBoundary ====
            if (Boundary.Count < 3)
                return true;
            double LatCenter = 0;
            double LongCenter = 0;
